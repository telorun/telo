...
function kindCompletions(
  ...
  for (const kind of kinds) {
    ...
    seen.add(kind);
    results.push({ label: kind, kind: "class", detail: "Telo resource kind" });
  }
  return results;
}
...
export async function buildCompletions(
  ...
  if (!ctx) return [];
  if (ctx.type === "kind") return kindCompletions(registry, ctx.docKind, ctx.yamlPath);
  ...
  if (ctx.type === "ref-name") {
    ...
    const refConstraint = definition?.schema
      ...
      : undefined;
    return refNameCompletions(text, ctx.refKind, refConstraint, registry);
  }
