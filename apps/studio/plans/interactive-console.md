...
### `apps/studio/src-tauri` (Rust runner)
...
- **Changesets.** This change touches `@telorun/docker-runner` and `apps/studio` (and the Tauri sidecar). Add one `.changeset` entry covering all affected published packages.
