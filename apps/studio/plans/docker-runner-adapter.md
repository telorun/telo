...
apps/studio/src/run/adapters/docker-api/     # new adapter; second citizen next to tauri-docker
...
The `apps/studio/src/run/` boundary from [run-adapters.md](./run-adapters.md) is unchanged — the new adapter is just another directory under `adapters/`, registered through `registry.ts`.
...
- **Prerequisite edit:** extend [pnpm-workspace.yaml](../../../../pnpm-workspace.yaml) to include the new package. The workspace currently lists `apps/studio` explicitly (not `apps/*`), so a new app under `apps/` is invisible to pnpm until declared. Replace the `apps/studio` line with `apps/*`, or add `apps/docker-runner` as a second entry — prefer the glob, since the repo will keep accumulating apps.
...
- `apps/studio/src/run/adapters/docker-api/` (adapter, config schema, sse client).
...
- Integration test that stands up the runner (via `dockerode` against the test host's daemon, not nested compose) and exercises the full editor adapter against it. Lives at `apps/studio/src/run/__tests__/docker-api.integration.test.ts` and is gated behind a `RUN_INTEGRATION=1` env so CI boxes without a daemon don't block.
