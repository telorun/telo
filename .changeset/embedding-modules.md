...
Add the `embedding` module — the `Embedding.Model` abstract plus the `Embedding.Query` / `Embedding.Passage` invocables that turn text into vectors — and `embedding-openai`, an OpenAI-compatible backend (`EmbeddingOpenai.Model`) speaking the `/embeddings` HTTP API directly.
