 ...
 * transport hashes exactly what its own `read()` verifies (HTTP the raw bytes,
 * OCI the UTF-8 text extracted from the tar layer), so a pin written at publish
 * always matches at import. A caller-side branch cannot know which, and
 * silently degrades the moment a transport is added — that is precisely how
 * `oci://` refs came to be published unpinned.
 */
export async function fetchManifestHash(ref: string): Promise<string> {
