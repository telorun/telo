...
- **Theme tokens stay per-medium.** `Ui.Theme` lives in `modules/ui`; `pdfmake` keeps `styles` / `defaultStyle` and `svg-chart` keeps `palette`. A neutral shared token module was considered and rejected — it couples three modules' release cadence to one vocabulary before any has shipped, and the duplication is a handful of literals.
