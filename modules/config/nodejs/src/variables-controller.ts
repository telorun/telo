...
type VariablesResource = RuntimeResource & {
...
class Variables implements ResourceInstance {
  private _values: Record<string, string> = {};

  constructor(
    private readonly resource: VariablesResource,
  ...
  async init(): Promise<void> {
    ...
    if (missing.length > 0) {
      throw new Error(
        `Config.Variables "${this.resource.metadata.name}": ` +
...
export function create(resource: VariablesResource, ctx: ResourceContext): ResourceInstance {
  return new Variables(resource, ctx);
}
