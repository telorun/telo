...
describe("Loader.loadFile url cache", () => {
  it("skips the source read on a second call for the same URL", async () => {
    ...
    const first = await loader.loadFile("oci://ghcr.io/telorun/foo@1.0.0");
    const second = await loader.loadFile("oci://ghcr.io/telorun/foo@1.0.0");

    expect(reads.get("oci://ghcr.io/telorun/foo@1.0.0")).toBe(1);
    expect(second).toBe(first); // identity-stable on cache hit
  });

  it("reparses from cached text when the compile mode differs", async () => {
    const { source, reads } = countingSource({
      "oci://ghcr.io/telorun/bar@1.0.0": {
        text: "kind: Telo.Library\nmetadata:\n  name: Bar\n  version: 1.0.0\n",
    ...
    await loader.loadFile("oci://ghcr.io/telorun/bar@1.0.0", { compile: false });
    await loader.loadFile("oci://ghcr.io/telorun/bar@1.0.0", { compile: true });

    // Same URL, different compile mode — the second call must not re-fetch.
    expect(reads.get("oci://ghcr.io/telorun/bar@1.0.0")).toBe(1);
  });
  ...
  it("issues exactly one read per distinct URL across loadGraph + loadModule cycles", async () => {
    const { source, reads } = countingSource({
      "app.yaml": {
        text: [
          ...
          "metadata:",
          "  name: Foo",
          "source: oci://ghcr.io/telorun/foo@1.0.0",
          "",
        ].join("\n"),
    ...
    await loader.loadModule("oci://ghcr.io/telorun/foo@1.0.0", { compile: true });
