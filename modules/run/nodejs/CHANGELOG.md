...
- adc8459: Add `Run.Choice` — a first-match decision table.
...
- aaa760d: Add three declarative control-flow kinds to `std/run`, each a thin binding wrapper over `Run.Sequence`'s `steps` body (sharing the step engine, now extracted into `engine.ts`):
