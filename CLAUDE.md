...
| `packages/ide-support/` | `packages/ide-support/CLAUDE.md` |
| `ide/vscode/` | `packages/language-host/CLAUDE.md` |
| `packages/language-server/`, `packages/editor-protocol/` | `packages/language-server/CLAUDE.md` (the protocol spec is `packages/editor-protocol/README.md`) |
| `packages/language-host/` | `packages/language-host/CLAUDE.md` |
...
- **Rust half of the polyglot runtime** — `kernel/rust/` (a second kernel), `cli/rust/` (a second implementation of the `telo` binary — one command, whichever kernel is behind it; it is not published while it hosts fewer controller formats than the Node build), `analyzer/rust/` and `templating/rust/` (the loading/tag halves of their Node counterparts), `sdk/rust/` (controller authoring surface) and `sdk/rust/abi/` (the `telorun-abi` C ABI). **File layout mirrors the Node packages one-for-one**, kebab-case becoming snake_case: a Rust file with no Node twin means one of the two layouts is wrong, and the header of every such file says why it is the exception. **A crate carries its Node twin's version** (`kernel/rust` ↔ `kernel/nodejs`, and so on — every `<x>/rust` crate whose `<x>/nodejs` is on the telo version line), written into its `Cargo.toml` and `Cargo.lock` entry by the version step (`scripts/version-line.mjs`, run by `version-packages` after `changeset version`) and checked by the changeset gate — the two halves are one artifact, exactly as a module is one version across its `telo.yaml`, `nodejs/package.json` and `rust/Cargo.toml`. `telorun-abi` is the only crate with no twin, and is versioned by the ABI generation its consumers name (`abi=telo-<n>`) rather than by its crate version. `kernel/rust` is deliberately narrow: `Telo.Invocable` only, local-path and anonymous `oci://` imports, `pkg:cargo` controllers built from a source checkout and `pkg:telo/local/dylib` controllers from a published artifact's layers, no CEL. `telorun-abi` exists because the kernel must **not** depend on `telorun-sdk` — that crate selects a controller backend by Cargo feature, and unification would compile the napi backend into the kernel binary.
- `apps/studio/` — desktop editor (React + Vite + Tauri)
- **Runners** — `packages/runner-core` (the backend-neutral `/v1` session contract) plus `apps/docker-runner`, `apps/k8s-runner` and the CLI's own local-process backend (`telo runner`) behind the `RunnerBackend` seam; a session is a one-shot `run` or a continuously running `watch` workspace. A backend is hosted by the artifact that owns its substrate, and the session config is the runner's own vocabulary rather than the contract's.
- `apps/hub/` — federated discovery hub (declarative Telo app) serving module search and the `search_resources` MCP tool
- `packages/ide-support/` — editor feature builders (completion, hover, rename, quick fixes, diagnostics assembly), served to editors by the `@telorun/language-server` engine and called in process by studio's structured views
- `packages/language-server/` — telo's editor engine: every diagnostic and editor feature of ONE telo version as a single self-contained, I/O-free ES module (`dist/language-server.mjs`) speaking LSP from `serve(port)`; `@telorun/language-server@X` is telo `X`'s engine, and an editor host runs the version it chose in a worker
- `packages/language-host/` — the published, browser-safe, version-agnostic editor side: the version catalog (npm), the selection rule over `requires: telo:` intervals, verified engine download and caching, and the router presenting one LSP endpoint across engines; every editor host runs on it
- `packages/editor-protocol/` — the published, zero-dependency contract between an editor host and an engine: the `telo/*` requests the host serves, the `telo/requirements` notification, the protocol generation (`teloEditorProtocol`); its README is the spec
- `ide/vscode/` — VS Code extension: an LSP client over `@telorun/language-host`, running the chosen telo version's engine in a `worker_threads` worker (the bundled engine ships in the `.vsix`)
...
- `variables` / `secrets` — each entry binds `env:` and/or (variables only) `arg:`, plus `type:`, optional `default:` and further JSON Schema keywords; resolved at `kernel.load()` into `variables.X` / `secrets.X`.
- **The command line is the root Application's**: `telo run [options] <path> [arguments]`, every token after the path read against the `arg:` bindings of its `variables:` / `ports:` entries (a flag, `{ flag, short }` or `{ position }`), the first source with a value winning — supplied by name, then argv, then env, then `default:`. Undeclared tokens are refused before boot and `--help` prints the usage; no controller reads argv. Runners and the studio supply inputs through env only, so a port always binds `env:` and a variable bound by `arg:` alone needs a `default:`. Spec: `kernel/specs/application-arguments.md`.
- `ports` — **Application-only** map of inbound ports (`env:`, optionally `arg:` beside it, `protocol: tcp|udp`, `default:`), read as `ports.X`; a runner knows them before boot.
...
- `variables` / `secrets` — JSON Schema property map; the public contract for importers. No `env` or `arg` access.
...
- **`inputs`/`outputs` are always VALUES; `inputType`/`outputType` are always SCHEMAS.** A contract resolves instance → nearest along `extends` and replaces rather than merges; the kernel binds it at `create()` and fails with `ERR_INPUT_INVALID` / `ERR_OUTPUT_INVALID`. `Telo.JsonSchema` is a kernel built-in (`modules/type` is deprecated). `throws:` is the contract's third part: an abstract may declare a literal `codes` list, and the nearest ancestor's list is a ceiling every descendant's codes must fall within (`THROWS_NOT_SUBSTITUTABLE` / `ERR_THROWS_NOT_SUBSTITUTABLE`); a dispatch through a kind-only library input resolves to that list, unbounded when none is declared. Spec: `kernel/specs/invocation-contract.md`.
...
- `x-telo-resource-rules` — CEL rules relating the fields of one resource (`RESOURCE_RULE_VIOLATED`); a rule's `resolve:` reads the declarations its own reference slots name, one level deep. Guide: `docs/extend/resource-rules.md`.
...
- `x-telo-type` — what a value IS: `Telo.TcpPort`, `Telo.UdpPort`, `Telo.Uint64`, `Telo.Bytes`, `Telo.Timestamp`, `Telo.Duration`, `Telo.Stream` (`{ name: Telo.Stream, of: … }`), `Telo.HostPath`; an instance type is written in YAML in its plain encoding (RFC 3339, `"5400s"`, base64url); a named shape is a `!ref` to a `Telo.JsonSchema`. A union with an instance branch must use `anyOf`, never `oneOf`.

## Host paths and module files

**A path is anchored by what DECLARES it, never by the controller reading it.** `Telo.HostPath` is an absolute path on the machine running the app — in CEL a nominal type of its own, not a `string`: extended with `.joinPath('sub/dir')` (the host's separator at runtime, a `Telo.HostPath` statically), read as text with `string(path)`, and a plain string at a host-path slot is a static error, however it was built: a relative one is refused wherever it is held (`HOST_PATH_RELATIVE` at check, with a `!module-path` repair; `ERR_HOST_PATH_RELATIVE` for a computed one at creation), except in an Application `variables:` / `secrets:` entry typed `Telo.HostPath`, whose env value or `default:` the kernel resolves against the working directory at load — the directory `.env*` is read from, and a double-clicked packaged app's own. Nothing else anchors: a Library's variable is its importer's value (supplied absolute), and a relative `default:` at a host-path node in a kind's schema or a Library variable is `HOST_PATH_RELATIVE` at the declaring doc — a blueprint's host-path field is required, and the consuming app supplies it from its own host-path variable. A string expression at a host-path slot is `HOST_PATH_UNTYPED_SOURCE` (or `HOST_PATH_RELATIVE` when its text starts with a relative literal), at a resource field and where an import supplies `variables:`; a relative literal an import passes is `HOST_PATH_RELATIVE`; a value of undeclared type (`dyn`) is left to the runtime's refusal at creation. A stdlib slot holding a path on the host declares `x-telo-type: Telo.HostPath` — `Http.Static.root`, every Fs kind's `cwd`, `SQLite.Connection.file` (`anyOf: [{const: ":memory:"}, HostPath]`), `DurableJournalFile.Journal.directory`.
...
2. **The bound is the surface generation — the version of the telo version line.** `node scripts/generate-telo-version.mjs` writes `TELO_SURFACE_VERSION`, which is the line's version (the highest version in the changesets `fixed` group holding `@telorun/analyzer`) plus the strongest pending changeset bump for any member — so on the commit that adopts new syntax it is the *unreleased* version that will ship it. **Forward-declaring is correct and expected**; `telo release check` reports such an edge as pending rather than trying to run it, and `publish` refuses it only once the release exists and the range is genuinely refuted.
...
- `telo check` vs kernel agreement → `tests/check-run-agreement.yaml`. **A guard enforced in a controller with no analyzer twin is a manifest that passes `telo check` and fails at boot** — a new refusal belongs in both halves, and in both halves of this suite. **The suite is for refusals the kernel and analyzer own** (built-in docs, annotations, formats, the manifest grammar). A module's own controller guard twinned by its `x-telo-resource-rules` / `x-telo-referrer-rules` is one module agreeing with itself: pin it the same two ways (refused at the declaring library, `runFails:` from a silent consumer) in that module's `tests/`, never here — otherwise the root suite grows with the stdlib and a module's own suite misses its own divergence.
...
- Template bodies → `template-body.ts`, `validate-template-body.ts`, `template-forward.ts`, `validate-template-forward.ts`, `kernel/nodejs/src/controllers/resource-definition/resource-template-controller.ts`
...
- `pkg:npm/@telorun/<pkg>@<ver>?local_path=./nodejs#<export>` — a published npm package; in this repo only the deferred modules (`image`, `starlark`) use it. Don't add new ones.
...
- **npm packages → changesets.** Every change to a published `@telorun/*` package MUST ship a changeset. Add one file under `.changeset/` (one per logical change, listing every affected package) and `git add` it. Use `pnpm changeset add --empty` when a change genuinely needs no release. The gate is `scripts/check-changeset-status.mjs`, not `changeset status` directly: private `-build` packages are versionable, and every package under `modules/*/nodejs/` is now module-owned (its version is its module's), so both are filtered out; only a changed **published, non-module** package with no changeset fails. The same module-owned set is in `.changeset/config.json`'s `ignore`, so the two ledgers never write the same field. **A published package whose build inlines workspace code declares every inlined workspace package — the full closure — in a `teloInlines` array in its `package.json`, and its build verifies the list against the bundle** (`verifyInlines`, exported by the gate script; `@telorun/language-server` is the case). The gate then fails when a changed package is listed by a published package that is not releasing, and on a `teloInlines` name that is not a workspace package; it also fails when a Rust twin's version differs from its Node twin's.
- **The telo version line.** `@telorun/sdk`, `@telorun/templating`, `@telorun/analyzer`, `@telorun/kernel`, `@telorun/cli`, `@telorun/ide-support` and `@telorun/language-server` are ONE changesets `fixed` group: **a changeset naming any member releases all of them at one version**, and that version is the manifest surface generation (`TELO_SURFACE_VERSION`, what `requires: telo:` is written against). **Membership test: a package belongs iff its content is bound to exactly one telo generation** — the analyzer's verdicts, the kernel's behaviour, the engine's answers. A contract spoken ACROSS generations (`@telorun/editor-protocol`, a host package) stays off the line, versioned on its own. The Rust twins of line members carry the same version (Monorepo Structure). `scripts/generate-telo-version.mjs` refuses to run when the analyzer is in no `fixed` group.
- **An engine's identity is its release.** `@telorun/language-server` reports `X` in its handshake only when it is the release `X`; built while a changeset of the line is pending it reports `X+unreleased` (generated beside `TELO_SURFACE_VERSION`), which editors never mistake for a published engine, and its `prepack` refuses a tarball whose engine does not report exactly its package version — so it publishes only after the Version PR.
- **Telo modules → `telo release`.** See `docs/extend/releasing-modules.md`. A module has **one version** across `telo.yaml`, `nodejs/package.json` and `rust/Cargo.toml`, and one changelog; `apply` moves the crate's entry in the governing `Cargo.lock` with it, because a lockfile left behind makes every `cargo --locked` invocation re-resolve over the network, which `--locked` forbids. Write a fragment with `telo release add --module modules/<name> --kind Fixed --body "…"` (`.changes/pending/*.yaml` — one file, several modules, one body, with the kinds `Added | Changed | Deprecated | Removed | Fixed | Security`). **Never write or edit a fragment file by hand** — to change one, delete it and run `telo release add` again: the command serializes the body safely, while a hand-written plain YAML scalar holding `: ` or quotes stops parsing as a string and fails `telo release check` (`'body' must be the changelog line`). `Changed` and `Removed` induce a major bump and `telo release check` rejects them: a breaking module change is written as `Added`, with the break described in the body. Never hand-edit `metadata.version`. A bundled module takes no changeset; its version moves when its payload digest does, and `telo release check` warns (`CHANGELOG_ENTRY_REQUESTED`) when a module's own files changed with no fragment naming it.
