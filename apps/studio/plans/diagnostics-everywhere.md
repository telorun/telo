...
- **No new diagnostic source.** Everything still comes from `analyzeWorkspace` in [analysis.ts](apps/studio/src/analysis.ts). No new analyzer passes, no runtime-error plumbing.
...
3. **Popover over tooltip.** Use the existing Radix `Popover` ([ui/Popover.tsx](apps/studio/src/components/ui/Popover.tsx)). Hover tooltips fight with text selection; click-to-open gives us copy UX for free.
...
New file: `apps/studio/src/diagnostics-aggregate.ts`
...
`WorkspaceTree` needs the set of files each module spans (owner + partials) so it can call `summarizeFiles`. That set is already discoverable — [analysis.ts:46-60](apps/studio/src/analysis.ts#L46-L60) walks `manifest.resources[].sourceFile` for the same purpose. Extract that logic into a small shared helper (`getModuleFiles(manifest)`) and call it from both sites.

## Components

### `DiagnosticBadge` (new)

Location: `apps/studio/src/components/diagnostics/DiagnosticBadge.tsx`
...
- Trigger: a small `<span>` with the severity icon (`●` error / `▲` warning / `i` info), optional count, in red / amber / blue (reuse the palette from [InventoryView.tsx:70-74](apps/studio/src/components/views/inventory/InventoryView.tsx#L70-L74)).
- Clicking the trigger opens a `Popover` whose content is `DiagnosticPopoverContent`.

### `DiagnosticPopoverContent` (new)

Location: `apps/studio/src/components/diagnostics/DiagnosticPopoverContent.tsx`
...
Implementation in [Editor.tsx](apps/studio/src/components/Editor.tsx):
...
[SourceView.tsx](apps/studio/src/components/views/source/SourceView.tsx) gains:
...
### Sidebar
- [WorkspaceTree.tsx:305-310](apps/studio/src/components/sidebar/WorkspaceTree.tsx#L305-L310) — module row (Application or Library). Summary: `summarizeFiles(state, getModuleFiles(manifest))`.
- [ResourcesSection.tsx:47](apps/studio/src/components/sidebar/ResourcesSection.tsx#L47) — per resource row. Summary: `summarizeResource(state, getModuleFiles(manifest), r.name)`. Passes the full file list so a resource declared in a partial is found even when the owner's path is selected.
- [DefinitionsSection.tsx:32](apps/studio/src/components/sidebar/DefinitionsSection.tsx#L32) — per definition row. `summarizeResource(state, getModuleFiles(manifest), r.name)`.
- [ImportRow.tsx:45](apps/studio/src/components/sidebar/ImportRow.tsx#L45) — import row. `summarizeResource(state, getModuleFiles(manifest), imp.name)`. Do **not** cross into the imported child module.

### Views
- [InventoryView.tsx:5-20](apps/studio/src/components/views/inventory/InventoryView.tsx#L5-L20) — delete the local `DiagnosticIndicator`. Replace with `DiagnosticBadge` in both the resources table ([InventoryView.tsx:134](apps/studio/src/components/views/inventory/InventoryView.tsx#L134)) and the definitions table ([InventoryView.tsx:202](apps/studio/src/components/views/inventory/InventoryView.tsx#L202)). The left-border row styling at [InventoryView.tsx:70-74](apps/studio/src/components/views/inventory/InventoryView.tsx#L70-L74) currently switches on `diagnostics.some(d => d.severity === Error)`; migrate it to read `summary?.worstSeverity === DiagnosticSeverity.Error` so row highlighting stays in sync with the badge.
- [ResourceCanvas.tsx:349](apps/studio/src/components/views/canvas/ResourceCanvas.tsx#L349) — next to the `<h2>` resource name, size="md".
- [DetailPanel.tsx:165-166](apps/studio/src/components/views/canvas/DetailPanel.tsx#L165-L166) — next to the panel title, size="md".
- [RouterTopologyCanvas.tsx:216](apps/studio/src/components/views/topology/RouterTopologyCanvas.tsx#L216) — next to node name, size="sm".
- [SequenceTopologyCanvas.tsx:881](apps/studio/src/components/views/topology/SequenceTopologyCanvas.tsx#L881) — next to node name, size="sm".

### TopBar
- [TopBar.tsx:52](apps/studio/src/components/TopBar.tsx#L52) — next to the module name breadcrumb, size="sm". Summary is the whole-module rollup (same as WorkspaceTree row).
...
2. **Aggregation helper** — `diagnostics-aggregate.ts` with `summarizeResource`, `summarizeFiles`, `summarizeWorkspace`; extract shared `getModuleFiles(manifest)` from the logic at [analysis.ts:46-60](apps/studio/src/analysis.ts#L46-L60).
