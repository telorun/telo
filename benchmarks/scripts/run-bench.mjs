...
// The benchmark suite emits its JSON report (and only its JSON report) on
// stdout in `format: json` mode — progress and status lines go to stderr —
// so the captured stdout should be JSON-only. We still scan candidate `{`
// positions defensively and try each balanced object, picking the first that
// parses, so a stray log line from the kernel or a controller can't poison
// parsing.
...
// Walks from `start` matching braces while respecting JSON string literals
// (so `{` or `}` inside a string doesn't change the depth). Returns the
// substring that contains the balanced object, or null if unbalanced.
