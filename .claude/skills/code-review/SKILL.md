...
- check if the change is larger than the feature: fixes to pre-existing issues the feature only passes through, the same fix applied to sibling kinds, tests proving a behaviour another test already proves, or tests of machinery another package owns. Recommend removal, not more coverage.
