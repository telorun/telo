...
- **`invoke({ method, params })`** writes a framed JSON-RPC request and awaits the matching response on stdout. Concurrent calls are correlated by ID, not call ordering. `ERR_MCP_SESSION_INVALID` is unreachable here — a dead session surfaces as `ERR_MCP_TRANSPORT` via child-process exit. A cancelled invocation sends the child `notifications/cancelled` for its request id and rejects with `ERR_INVOKE_CANCELLED`; errors the SDK reports outside any request (such as a cancellation it could not deliver) are logged at `warn`.
