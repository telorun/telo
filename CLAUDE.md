...
- `targets` — optional; run after all resources init. A flat boot sequence: each entry is a bare reference / `!ref` to a `Telo.Runnable` or `Telo.Service` (`run()`), a gated reference `{ ref, when? }`, or an inline invoke step `{ name?, invoke: <Invocable ref>, inputs?, when?, retry? }`. Inline steps call an `Invocable` via the shared `executeInvokeStep` leaf (SDK), with `steps.<name>.result` plumbed into later targets and `when`/`inputs` evaluated against the root scope. Ref-only — inline targets reference declared resources; control flow (`if`/`while`/`switch`/`try`), `with:` scopes, and callable `inputs`/`outputs` stay in `Run.Sequence`. A no-targets Application is valid when its work is carried by Services that auto-start on init.
