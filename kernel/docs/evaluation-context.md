...
- `resources` _(Dictionary/Map)_: References to instantiated resource objects in the module scope. Includes both locally-defined resources (e.g., an active database connection, an HTTP server instance) and imported modules (keyed by the user-defined alias, mapping to the exported properties of that module instance).
  - `resources.<name>.status` is the resource's **observed state** — what it learned while running, declared by its kind's `status:` block and readable only from fields that resolve while the application runs. `status` is a reserved key in every published value. See [Observed state](./observed-state.md).
