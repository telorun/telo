...
* Declarative schema. Sql.Table and Sql.Schema are the backend-neutral contracts behind it: Sql.Table for a declared table whose row shape projects to JSON Schema, and Sql.Schema for the grace-window surface — the version clock an irreversible drop is gated on, the reclamation policy gating it, and the observed state reporting what is still held. The shared runner ships through the module library: the diff, the ledger, the tombstones and the ordering are here, while every type vocabulary, DDL rendering, introspection and lock is the backend's. Sql.Migrations and Sql.Migration are removed and absorbed into each backend's Schema kind, which owns both migration phases as well as the declared tables, so the order between imperative and declarative schema change is defined rather than left to the author's targets list.
...
* Sql.Connection gains two methods. `bindsZone(zone)`: does THIS connection hold the
open executor for THAT zone. `hasOpenTransaction` answers the dispatch-time question —
is there one to execute on — while an attestation needs the membership question, and
an ambient check would claim membership of a region a caller's writes may not be in.
`executeUncommitted(sql, params)` runs a statement on the connection and never on an
ambient transaction, for a write that is a record ABOUT the work rather than part of
it and so must survive whatever the work was doing — a durable journal settling a run,
or releasing a claim. Joining stays the default, and is what makes exactly-once work.
