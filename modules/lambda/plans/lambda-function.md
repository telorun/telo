...
- **(Hard) Transport-neutral response rendering + shared outcome schema** ([../../../packages/http-dispatch/nodejs/plans/transport-neutral-response.md](../../../packages/http-dispatch/nodejs/plans/transport-neutral-response.md)). `@telorun/http-dispatch` owns one HTTP-dispatch concern end to end — both the *runtime* dispatcher and the *manifest* schema:

  - **Runtime half** (already shipped). `dispatchReturns` / `dispatchCatches` plus the `ResponseSink` interface live in [`packages/http-dispatch/nodejs/src/`](../../../packages/http-dispatch/nodejs/src/). Both `@telorun/http-server` and `@telorun/lambda` add `@telorun/http-dispatch` as a workspace dependency; the Lambda controller writes a `LambdaResponseSink` adapter (see [Shared dispatch](#shared-dispatch)) and calls the shared dispatch. The package is intentionally *not* placed inside `@telorun/sdk` — the SDK keeps its zero-runtime-deps posture (HTTP-shaped dispatch code does not belong in the install tree of every non-HTTP module).
  - **Manifest half** (POC landed for `Outcomes`; `Request` carrier and full http-server migration pending). [`packages/http-dispatch/telo.yaml`](../../../packages/http-dispatch/telo.yaml) is a `Telo.Library` publishing two `Telo.Definition`s (`capability: Telo.Type`):
...
metadata:
  name: HttpDispatch
source: std/http-dispatch@0.1.0
---
...
capability: Telo.Invocable
controllers:
  - pkg:npm/@telorun/lambda@0.1.0?local_path=./nodejs#http-api
inputType:
  kind: Type.JsonSchema
  schema:
    type: object
...
capability: Telo.Invocable
controllers:
  - pkg:npm/@telorun/lambda@0.1.0?local_path=./nodejs#sqs
inputType:
  kind: Type.JsonSchema
  schema:
    type: object
...
capability: Telo.Invocable
controllers:
  - pkg:npm/@telorun/lambda@0.1.0?local_path=./nodejs#direct
inputType:
  kind: Type.JsonSchema
  schema:
    type: object
