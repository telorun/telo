...
const AMBIENT_CAPABILITIES = new Set(["Telo.Provider", "Telo.Type"]);
 ...
 *  resources and other non-reference values. */
  ...
  registry.visitManifest(resources, {
    onRef: (e) => {
      ...
      if (NODE_CAPABILITIES.has(capability)) {
        edges.push({ from, to: targetName, label });
      } else if (AMBIENT_CAPABILITIES.has(capability)) {
        chips.push({ on: from, target: targetName, label });
      }
    },
    // Expanded map so refs nested behind x-telo-schema-from (e.g. encoders) are
    // surfaced as sites too, matching reference validation / the dependency graph.
  }, { expand: true });
