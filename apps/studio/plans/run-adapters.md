...
Async function. The workspace graph (modules, imports, `include` paths) is in memory and read synchronously; **file contents are not**: [ParsedManifest](apps/studio/src/model.ts#L31-L51) stores `include: string[]` (paths only), and `rawYaml` is populated only on parse failure. Partial files and even the main `telo.yaml` texts must be read via `WorkspaceAdapter.readFile`.
...
The existing [storage.ts](apps/studio/src/storage.ts) `saveState` / `loadPersistedState` round-trips a three-field `PersistedState` (`rootDir`, `activeModulePath`, `activeView`) explicitly projected at [storage.ts:20-24](apps/studio/src/storage.ts#L20-L24), and the file comment scopes it to "lightweight cross-session state." `deploymentsByApp` doesn't fit that envelope — it's a per-workspace map of structured data that should evolve independently from UI-focus state.
...
- Registered by extending [ViewId](apps/studio/src/model.ts#L104) with `"deployment"` and adding the same string to `VALID_VIEWS` in [storage.ts:7](apps/studio/src/storage.ts#L7) so `activeView` rehydration accepts it.
...
Full-canvas view, **not** a dockable bottom panel. When a session is active (or Run was just clicked against an unavailable adapter), the main content area in [Editor.tsx](apps/studio/src/components/Editor.tsx) renders `RunView` in place of the active view (topology / inventory / source / deployment). The [DetailPanel](apps/studio/src/components/DetailPanel.tsx) is hidden while `RunView` is shown — the run deserves the full width. The sidebar stays. Closing `RunView` (X button, or a `view: "previous"` control) restores the previous active view.
...
The existing [TopBar Run button](apps/studio/src/components/TopBar.tsx#L59) replaces its icon with a spinner while a session is `starting` or `running`. When the session reaches a terminal state and the user has closed `RunView`, a small coloured dot remains on the button (green for `exited: 0`, red for `failed` / non-zero exit / `stopped`) until the next Run click.

### Run button behaviour

Current stub in [Editor.tsx:220-223](apps/studio/src/components/Editor.tsx#L220-L223) is replaced by:
...
- Extend [AppSettings](apps/studio/src/model.ts#L10-L12) with `activeRunAdapterId: string` and `runAdapterConfig: Record<string, unknown>`. Persistence flows through the existing `saveSettings` / `loadSettings` path in [storage.ts](apps/studio/src/storage.ts) — run-adapter choice is a user preference, not workspace-scoped state, so it belongs alongside `registryServers` rather than in `EditorState`. Update [DEFAULT_SETTINGS](apps/studio/src/model.ts#L23) to seed the default adapter id. Per-adapter config is stored opaquely by id; the adapter's `configSchema` is the source of truth for its shape.
- [SettingsModal.tsx](apps/studio/src/components/SettingsModal.tsx) gets a new "Run" section:
...
- Otherwise → render via the existing [resource-schema-form/](apps/studio/src/components/resource-schema-form/) renderer, feeding `adapter.configSchema`.

`ResourceSchemaForm` does **not** currently accept per-field error props — [its props](apps/studio/src/components/resource-schema-form/index.tsx#L6-L15) are `schema / values / onChange / onFieldBlur / onParseStateChange / resolvedResources / rootCelEval / onSelectResource`. Getting `ConfigIssue[]` to the user therefore has two paths:
...
- Extend [eslint.config.mjs](apps/studio/eslint.config.mjs) with the `no-restricted-imports` rule described in *Module-boundary enforcement*.
...
- Extend [ViewId](apps/studio/src/model.ts#L104) with `"deployment"` and add the same string to `VALID_VIEWS` in [storage.ts:7](apps/studio/src/storage.ts#L7); add a fallback to `"topology"` when the persisted view is `"deployment"` but the active module is a Library.
...
Frontend ([apps/studio/package.json](apps/studio/package.json)):
...
Rust ([apps/studio/src-tauri/Cargo.toml](apps/studio/src-tauri/Cargo.toml)):
...
The "nothing outside `src/run/` imports from `src/run/adapters/` or `src/run/ui/`; only `src/run/index.ts` is a valid external import target" rule is enforced by lint, not convention. The current [eslint.config.mjs](apps/studio/eslint.config.mjs) only does `globalIgnores`, so PR 1 extends it:
