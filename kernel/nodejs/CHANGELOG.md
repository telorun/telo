...
- Updated dependencies [53e8f07]
- Updated dependencies [e42957e]
- Updated dependencies [e42957e]
  - @telorun/analyzer@0.84.0
...
- 95762d6: Add `telo run --startup-profile <file>` to write a low-overhead JSON breakdown of CLI-visible startup phases and per-resource create/initialization timings.

### Patch Changes

- Updated dependencies [95762d6]
  - @telorun/templating@0.22.0
  - @telorun/analyzer@0.80.0
...
- 21b3486: `telo package` — ship an application as one executable
...
- 814cd18: `ctx.runtime.run` takes a child's declared inputs by name, and a child is now reachable as a text channel.
...
- 390b998: Ship `telo` as a standalone executable, and fix what a distribution without `node_modules` exposed.
...
- Updated dependencies [0796a3c]
- Updated dependencies [0796a3c]
- Updated dependencies [0796a3c]
  - @telorun/analyzer@0.77.0
  - @telorun/templating@0.21.0
...
- Updated dependencies [3372dac]
  - @telorun/analyzer@0.75.0
...
- Updated dependencies [2d93519]
- Updated dependencies [8cfb01c]
- Updated dependencies [a9d05db]
- Updated dependencies [2d93519]
...
- 7658f43: A dispatch that misses because the runtime withdrew the resource is reported as a cancellation (`ERR_INVOKE_CANCELLED`) rather than as a missing resource.
...
- Updated dependencies [61ad85e]
- Updated dependencies [7658f43]
...
- ad690e8: `Kernel.reconcile()` brings a running kernel into line with an edited manifest,
...
- ad690e8: Resolving a sibling by name is now recorded, so a host reconciling a changed
...
- ad690e8: `EvaluationContext.unwindResources(names)` tears down some of a context's
...
- ad690e8: `impactClosure` answers the second question the create-time reference edges can
...
- ad690e8: A resource is now torn down before every resource it holds, across an import
  ...
  resource holding `!ref Alias.name` unwound _after_ its provider was already gone
...
- 9f1d446: Key the npm controller install root per runner, so two runners over one workspace no longer corrupt each other's tree.
...
- 831c0c4: A union-typed slot can hold a whole-field CEL expression. The compiled-value
...
- 07fca98: Controller log records carry their resource identity again, and a detached task failure is no longer invisible.
...
- e7853d5: Controller resolution is deferred to a kind's first instantiation, matching the
...
- 0b971d6: `ctx.runtime.check()` no longer analyzes a manifest that failed to parse, and no
...
- bf324d2: Fix `Maximum call stack size exceeded` when a template passes its caller's resource down to a child.
...
- 6376a66: Fix a `with:`-scoped resource that shares a name with a module-level one.
  ...
  - **`Run.Sequence` rejected an injected scope target.** `scopeTargetName` read only
    ...
    name the entry _is_ injected as a live instance, which it treated as an
...
- cae53b0: Allow `metadata.categories` on module and kind docs — an unordered list of
...
- 721a241: Fix controller bundling duplicating a package's own shared modules. `telo install`
