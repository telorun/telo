...
- `Run.Throw` — an invocable that throws an `InvokeError` at a specific call site
...
A `catch` block that falls through without re-throwing *absorbs* the error. A `catch` block that ends in a `Run.Throw` step re-raises it — the step's `inputs.code` determines which codes propagate out of the sequence.

## `Run.Throw`

`Run.Throw` is a `Telo.Invocable` that takes `{ code, message, data? }` and throws the matching `InvokeError`. It is declared with `throws: { passthrough: true }` — meaning its effective throw union is whatever `inputs.code` resolves to at the call site.

| Form                                | Statically resolves to            |
|-------------------------------------|-----------------------------------|
| `inputs.code: "UNAUTHORIZED"`       | `{ UNAUTHORIZED }`                |
| `inputs.code: "${{ 'FOO' }}"`       | `{ FOO }`                         |
| `inputs.code: "${{ error.code }}"` inside a `catch` | the enclosing `try`'s propagated union |
...
1. Resolve the step's `invoke.kind` via the definition registry.
2. If the target is another `inherit: true` composer, recurse (memoised by manifest name, cycle-safe).
3. If the target is `passthrough: true` (e.g. `Run.Throw`), resolve at this specific call site.
4. Otherwise, use the target's declared `throws.codes`.

Inside a `try` / `catch`, the catch block's throws *replace* the try block's (a `catch` that runs to completion has absorbed the try error; a `catch` that ends in `Run.Throw` re-raises whatever it decides).
...
- **`passthrough: true` outside `catch:`** — using `${{ error.code }}` on `Run.Throw` outside an enclosing `catch` block is rejected (no enclosing try to source the union from).
