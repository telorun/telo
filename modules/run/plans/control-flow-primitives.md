...
Analyzer context is wired through schema annotations. Each kind's `steps`/`outputs`/`condition`/`catches` fields carry an `x-telo-context` declaring their scope variables: `inputs`/`iteration`/`previous` as `dyn`, `index` as integer, `items` as array; `error` rides the existing `x-telo-error-context` on `catches`; and `steps.<name>.result` is strictly typed via `x-telo-step-context` exactly as in `Run.Sequence` (verified: an unknown `steps.X` in a body step or in `outputs` is a `CEL_UNKNOWN_FIELD`).

**`item` is typed automatically from `collection`** via a new generic analyzer annotation, **`x-telo-context-element-from: "collection"`** (`analyzer/nodejs/src/validate-cel-context.ts`). When `collection` is a member-access chain into the resource's typed `inputs` contract, the resolver walks the chain to the array and exposes its `items` as `item`'s schema — so `item.<unknownField>` is a `CEL_UNKNOWN_FIELD`. List-literal / comprehension / untyped-source collections fall back to `dyn` (no false positives). Covered by `analyzer/nodejs/tests/element-context.test.ts`.

Deferred (follow-up, not blocking): extend the inference beyond `inputs.*` chains (e.g. `steps.*`-rooted collections, which need the step context threaded in) and derive `Run.Projection`'s own `outputType` as `array<element>` from its `outputs`.
