...
The panel header carries an explicit **"Open in canvas"** button that promotes the current peek target to `graphContext` (swaps the main canvas). This is the only path from peek to navigation. Wire the button to the existing `onNavigateResource(kind, name)` callback from [types.ts:10](apps/studio/src/components/views/types.ts#L10) — no new callback needed; the sidebar and inventory view already use it with the same semantics.

Single-level for now: panel shows the most recent sub-selection, no navigation stack. `selectedResource` is cleared when the active module changes — mirroring how `graphContext` is cleared at [Editor.tsx:213](apps/studio/src/components/Editor.tsx#L213) — so peeking is always relative to the current main canvas.
...
- **Browser-safe.** No new Node.js imports in `apps/studio` or `analyzer/nodejs`.
...
New pure helper in `apps/studio/src/components/views/topology/bindings.ts`:
...
New component `apps/studio/src/components/views/resource-canvas/ResourceCanvas.tsx`.
...
In [Editor.tsx](apps/studio/src/components/Editor.tsx) (and wherever the sidebar triggers selection), drop the "has topology → set graphContext" gating. Every resource selection updates `graphContext` so the main canvas renders something.
...
Strip the form-duplicating guts of [DetailPanel.tsx](apps/studio/src/components/DetailPanel.tsx). Replace with a wrapper that renders `ResourceCanvas` for `selectedResource` (the current sub-selection), distinct from `graphContext` (the main canvas resource).
...
**State-model divergence — call out during build.** [types.ts:7-10](apps/studio/src/components/views/types.ts#L7-L10) already declares `selectedResource`, `graphContext`, `onSelectResource`, and `onNavigateResource`, but the selection handlers in [Editor.tsx](apps/studio/src/components/Editor.tsx) currently update both contexts together (see the `graphContext: { kind, name }` write at [Editor.tsx:342](apps/studio/src/components/Editor.tsx#L342) whenever selection changes). This step decouples them cleanly:
...
`apps/studio/ARCHITECTURE.md` was never checked in, so there are no edits
