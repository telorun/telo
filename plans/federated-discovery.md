...
**Required follow-up before a Phase 2 production build — flip the pgvector import to its published ref.** The hub imports `vector-store-pgvector` by relative path (`../../modules/vector-store-pgvector`) because the module is not yet published. That resolves in the repo-mounted compose `development` target (local dev + the hub e2e CI job), but the production Docker `build` stage copies only `apps/hub/telo.yaml` and runs `telo install`, so the relative path escapes the build context and the build **fails loudly** until the import is flipped to `std/vector-store-pgvector@0.1.0`. The flip is gated on publishing the module (its changeset/changie fragments drive that release) and is **not** caught by `telo check` (which resolves the relative path locally). Track it as a release-ordering step: publish the module, then flip the import.

**Fast-follows (post-v1, already scoped out in Decisions):** a **backfill pass** that embeds a module's latest version when its vectors are absent (so enabling Phase 2 over an existing relational DB needs no wipe — today embedding only runs inside the digest-change branch, so unchanged-digest modules stay unembedded until a re-track from clean); the ownership-challenge verified-publisher badge on the same `/register` flow; a locally-synced offline search index (`helm search repo` analog); promoting the CLI read verbs to a reusable declarative `Module.*` client kind once a second in-manifest consumer lands.
