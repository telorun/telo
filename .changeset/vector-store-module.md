...
"@telorun/vector-store-memory": minor
---

Add the `vector-store` module — the `VectorStore.Store` abstract (a backend-pluggable vector index) plus the `Record` / `Match` / `Removal` invocables that upsert, query, and delete vectors against any store, mirroring the `cache` / `cache-memory` family.
