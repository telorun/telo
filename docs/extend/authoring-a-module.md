...
> **Reading an `x-telo-ref` slot.** The kernel injects the live instance into a ref field before `init()`, so `this.resource.store` is usually already the target. Read it with `ctx.resolveRef(this.resource.store, isMyContract, () => 'My.Kind "name": \'store\'', 'std/cache#Store')` rather than a bare lookup: it duck-type-checks the target with your guard — so a mis-wired ref fails with a coded, actionable error naming the resource, the slot and the expected contract instead of `undefined is not a function` — and still resolves the slots injection doesn't reach. Type the field as `MyContract | KindRef<MyContract>`.
