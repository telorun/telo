...
export function detectContext(
  ...
  if (kindLineMatch) {
    ...
    if (character >= valueStart) {
      if (indent === 0) return { type: "kind" };
      if (docKind) {
        const yamlPath = buildYamlPath(lines, line, start, indent);
        return { type: "kind", docKind, yamlPath };
      }
    }
  ...
  // type extra spaces) is intentional. The `inferIndentForBlankLine`
  // heuristic is kept as a fallback when the column is somehow unavailable,
  // but trusting `character` lets the user reach root level (col 0) even
  // when the previous non-blank line was nested.
  const indent =
    trimmed === ""
      ? character
