#!/usr/bin/env node
// Auto-detects changed workspace packages (excluding Telo modules) via git,
// writes a changeset file, then delegates versioning/publishing to @changesets/cli.
// Telo modules are released separately via: node scripts/release-modules.mjs
//
...
if (!skip("version")) {
        ...
        dirs = dirs.flatMap((d) => {
          try {
            return readdirSync(d)
              .filter((e) => {
                try {
                  return statSync(join(d, e)).isDirectory();
                } catch {
                  return false;
                }
              })
  ...
  const packages = patterns.flatMap(expandPattern).flatMap((dir) => {
  ...
  // Module npm packages (modules/*/nodejs/) are released via release-modules.mjs — exclude here
  const modulePackageDirs = new Set(
    readdirSync(join(ROOT, "modules"))
      .map((m) => join(ROOT, "modules", m, "nodejs"))
      .filter((d) => existsSync(d)),
  );
  const nonModulePackages = packages.filter((p) => !modulePackageDirs.has(p.dir));
  ...
  for (const file of changedFiles) {
    ...
    let best = null,
      bestLen = 0;
    for (const pkg of nonModulePackages) {
