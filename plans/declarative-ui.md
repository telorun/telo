...
**Two modules, split on the spec.** `modules/ui` owns the vocabulary and ships **zero client code**. `modules/ui-react` owns `Ui.App` (a `Telo.Mount`) and the browser bundle. `modules/crud` imports `ui` only — a backend module declares UI without depending on any renderer, which is what makes the spec a real contract and leaves an SSR-HTML renderer possible later. That renderer is for this spec only: `plans/svg-chart.md` is server-rendered SVG with CEL accessors and is deliberately not a back end for a client-side chart, so an interactive `Ui.Chart` is its own vocabulary here — a static chart embeds that module's markup through the `svg` node instead.
...
- **Theme tokens stay per-medium, but a font does not.** `Ui.Theme` lives in `modules/ui`; `pdfmake` keeps `styles` / `defaultStyle` and `svg-chart` keeps `palette`. A neutral shared token module was considered and rejected — it couples three modules' release cadence to one vocabulary before any has shipped, and the duplication is a handful of literals. **A font is the deliberate exception** (`modules/font`, in `plans/svg-chart.md`): the duplication there is a payload plus a parser rather than literals, and a chart's layout is computed against advance widths a page must then actually serve, so the two must provably be one file. A theme's font token is a `!ref` to a `Font.Family`; `Ui.App` serves its faces and emits the `@font-face` from that declaration, so the family name in CSS is never a second hand-written string.
